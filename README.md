...
- C# : **마이크로소프트**가 개발한 객체지향 프로그래밍 언어
    ...
    - 1985년, 비야네 스트롭스트롭이 C++을 개발(너무 어려움, 1980년 초반 개발시작)
...
- 순서
    ...
    3. 추가 정보
        ...
        - LTS(Long Term Support) : 개발자가 기능, 보안업데이트를 지원
        - 최상위 문 사용 안함 체크박스 선택
    4. 스케폴딩 후 IDE 오픈
...
- 각 IDE별 한글 깨짐 해결방법
    - VS Code > 오른쪽 하단 상태표시줄의 UTF-8 클릭 > 인코딩 하여 다시 열기 또는 인코딩하여 저장 실행
...
- 기본 문법
    ...
    4. 상속, 다형성, 추상클래스, 인터페이스 - [소스](./day03/Day03Study/SyntaxWinApp02/Computer.cs)
        ...
        - 인터페이스 : 계약관계를 정의하고 구현없이 선언만 포함한 클래스의 일종
            - 추상클래스 개선한 방법
            - 추상클래스는 구현을 가질 필요 없이 하위에서 상속된 자식클래스가 다시 구현하면됨
...
- 기본문법
    ...
    2. 예외처리
        ...
        https://github.com/user-attachments/assets/ccf40b46-6ce0-4c54-bb49-1d2c019ef499
...
- 고급 문법
    ...
    3. 속성 - Property
        ...
        - UI 클래스(속성) : UI 상태나 외형, 기능 컨트롤
            - 폼 디자인, 코드 둘 다 사용
        - Form.Designer.cs 내용은 폼 디자인에서 마우스나 디자인 설정을 변경하는 값으로 되도록이면 수정하지 말 것
    ...
    5. 제네릭 : Generic
        ...
        - 제네릭 메서드
        - 제네릭이 없으면,
        - 실제, Console.cs의 WriteLine()이 그 증거
...
### 윈앱 컨트롤 5
- `BackgroundWorker` : 화면 뒷단에서 작업할 일 중 스레드 처리가 되어야 할 부분은 손쉽게 동작시켜주는 컨트롤(화면표시X)
    - BgW~ 이름을 사용
...
- `TreeView` : 탐색기 하드디스크, 폴더를 나타내는 부분과 동일한 컨트롤
    - Trv~
    - ImageList : 아이콘 이미지 설정
    - AfterSelect, AfterExpand, AfterCollapse 이벤트를 주로 사용

- `ListView` : 탐색기 오른쪽 목록을 표시하기 위해 사용하는 컨트롤
    - Lsv~
...
- `MenuStrip` : 프로그램 메뉴관리
    - Mnu~ 이름시작. 사용하는 메뉴는 반드시 이름을 변경할 것. (예: 끝내기XMenustrip)과 같이 이름이 생성
...
- XAML
    ...
    - 태그형태로 디자인. <열린태그></닫힌태그>, <태그 내 다른 태그없을때 />
    ```xml
...
- 각 프레임워크별 장점
    ...
    - WPF
        ...
        - GPU를 사용하여 GUI 랜더링을 해 윈앱보다 성능이 뛰어남
...
- 컨트롤 디자인, 리소스 - [소스](./day08/Day08Study/WpfStudyApp04/MainWindow.xaml)
    ...
    - ResourceDictionary - [소스](./day08/Day08Study/WpfStudyApp04/App.xaml)
        - App.xaml 리소스 계속 추가하면 유지보수가 어려워짐. 대안으로 리소스사전 생성
...
- 기본 사용법 - [소스](./day09/Day09Study/WpfStudyApp07/MainWindow.xaml)
    ...
    - Grid 내 타이틀바 추가
        ...
        ```
    - Theme를 Dark로 했을 경우
        - Window 전체를 Background를 어두운색으로 지정 후 작업
